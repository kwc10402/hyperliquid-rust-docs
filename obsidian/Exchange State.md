...
| 1 | `scl` | spot_clearinghouse | SpotClearinghouse(12) | {meta, user_states, daily_exchange_scaled_and_raw_vlms, token_to_genesis, token_to_max_supply, token_fees_collected, native_token, spot_dusting_opted_out_users, evm_transfer_escrows, token_to_pending_evm_contract, pmr, soh}. **OutcomeTracker** lives at `scl.meta.outr` with `fee_scale` field (`outr.fs`) controlling outcome fee rates. |
